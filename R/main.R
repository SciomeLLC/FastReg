...
FastReg <- function(config.file = NULL, ...) {
  ...
  for (stratum in 1:nstrata) {
    ...
    X <- createDesignMatrix(
      df = covar.df[ind.set, , drop = FALSE], covariates = args[["covariates"]],
    ...
    if (args[["poi.block.size"]] == 0) {
      ...
      poi.block.size <- estimate.poi.block.size(num.poi = num.POI, num.ind = length(ind.set), poi.type = args[["poi.type"]], max.cores = args[["num.cores"]])
    } else {
    ...
    for (block in 1:num.poi.blocks) {
      ...
      if (args[["POI.type"]] == "genotypes") {
        ...
        fwrite(filter.res,
          file = file.path(args[["output.dir"]], paste0("POI-Summary", outfile.Suffix, ".tsv")),
          sep = "\t", append = (block > 1)
        )
