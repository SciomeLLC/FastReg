...
linearRegression <- function(Y, G, X=NULL, Z=NULL, G.transform.fun=NULL, G.filter.fun=NULL, G.transform.args=NULL, G.filter.args=NULL, no.intercept=FALSE) {
  ...
  One.S <- matrix(1, nrow=1, ncol=nS, dimnames=list(NULL, rownames(Y)))
  ...
  Y <- Y%*%One.V;
    ...
    XtY[1:nX,] <- matmult2D(f_t, X, Y);

    for(j in 1:nZ) {
      XtX[1:nX,nX+j,] <- matmult2D(f_t, X, G*Z[,j]);

      for(d in 1:nX) XtX[nX+j, d,] <- XtX[d,nX+j,];
	    for(k in 1:nZ) XtX[nX+j,nX+k,]   <- matmult1D(f_t, G*Z[,j],G*Z[,k]);

	    XtY[nX+j,] <- matmult1D(f_t, Y, G*Z[,j]);
    ...
    beta <- matmultSolve(a=XtX, b=XtY);
		...
		mse <- matmult1D(f_t,Y-mu)/DF;

		remove(list="mu");

		XtX <- matmultElement(XtX, mse);
