...
//implements POSIX function getline for portability - adds gz read capability, skips empty lines
...
static int preprocess_datafile(FILE *datafile, gzFile gzdatafile, struct fastR_user_params *par, struct preprocess_info *pi) {
	ssize_t nread;
	char *line=NULL;
	char *buff, *pch=NULL;
	size_t colnamessize;
	...
	while(pch!=NULL) {
		...
		if(j==colnamessize) {
			colnamessize*=2;
			pi->colnames=(char**)realloc(pi->colnames, colnamessize*sizeof(char*));
		}
		pch=separate(&buff,par->delim);
	}
	pi->dimensions[1]=j;
...
static int initialize_dims_h5(struct dim_vars **dvars, struct hdf5_vars **h5vars, struct fastR_user_params *par, struct preprocess_info *pre, struct read_buffers *readbuff, char *placeholder) {
	...
	if(par->transpose==0) {
		...
		if(par->single==1) {
			...
			chunked=Chunker::estimate_num_files((int)pre->dimensions[1], (int)pre->dimensions[0], par->server_threads, par->server_memory);
		...
		pre->row_dim=(par->data_buffer_max-filecount*sizeof(float**))/(1024+sizeof(char*)+filecount*sizeof(float*)+pre->dimensions[1]*sizeof(float));
		if(pre->row_dim>=par->chunk_edge) {
			pre->row_dim=par->chunk_edge;
		} else if(pre->row_dim>0) {
			bytes=filecount*sizeof(float**)+par->chunk_edge*(1024+sizeof(char*)+filecount*sizeof(float*)+pre->dimensions[1]*sizeof(float));
			gb=((float)bytes)/(1024*1024*1024);
			gb+=0.01;
			fprintf(stderr, "Warning: sub-optimal data buffer size selected. For best performance specify\n    %.2f Gb or higher\n", gb);
		} else {
			bytes=1024+sizeof(char*)+filecount*sizeof(float**)+filecount*sizeof(float*)+pre->dimensions[1]*sizeof(float);
			gb=((float)bytes)/(1024*1024*1024);
			gb+=0.01;
		...
		*h5vars=(struct hdf5_vars*)malloc(filecount*sizeof(struct hdf5_vars));
		...
		if(remainder>0) {
			...
			(*dvars)[i].vals_chunk_dims[0]=(par->chunk_edge > remainder ? remainder : par->chunk_edge);
			(*dvars)[i].vals_chunk_dims[1]=(par->chunk_edge > remainder ? remainder : par->chunk_edge);
		} else {
			(*dvars)[i].vals_memspace_dims[1]=file_poi;
			(*dvars)[i].vals_dataspace_dims[1]=file_poi;
			(*dvars)[i].vals_chunk_dims[0]=par->chunk_edge;
		...
		datasize=filecount*sizeof(float**)+filecount*pre->row_dim*sizeof(float*)+pre->row_dim*pre->dimensions[1]*sizeof(float);
		...
		for(i=0;i<filecount;i++) {
			...
			for(j=0;j<pre->row_dim;j++) {
				readbuff->val_buffer[i][j]=(float*)(rowp+i*pre->row_dim*file_poi+j*(*dvars)[i].vals_memspace_dims[1]);
			}
		}
		...
		if(par->single==1) {
			...
			chunked=Chunker::estimate_num_files((int)pre->dimensions[0], (int)pre->dimensions[1], par->server_threads, par->server_memory);
			file_poi=chunked.num_poi;
		}
		if(file_poi<par->chunk_edge) {
			par->chunk_edge=file_poi;
		...
		Rcpp::Rcout << "POI per H5 file: " << file_poi << "\nH5 File Count: " << filecount << "\n";
		pre->row_dim=(par->data_buffer_max-sizeof(float**))/(1024+sizeof(char*)+sizeof(float*)+pre->dimensions[1]*sizeof(float));
		if(pre->row_dim>=par->chunk_edge) {
			pre->row_dim=par->chunk_edge;
		} else if(pre->row_dim>0) {
			bytes=sizeof(float**)+par->chunk_edge*(1024+sizeof(char*)+sizeof(float*)+pre->dimensions[1]*sizeof(float));
			gb=((float)bytes)/(1024*1024*1024);
			gb+=0.01;
			fprintf(stderr, "Warning: sub-optimal data buffer size selected. For best performance specify\n    %.2f Gb or higher\n", gb);
		} else {
			bytes=sizeof(float**)+1024+sizeof(char*)+sizeof(float*)+pre->dimensions[1]*sizeof(float);
			gb=((float)bytes)/(1024*1024*1024);
			gb+=0.01;
			fprintf(stderr, "Error: selected data buffer size too small to retain a single row. Select\n    %.2f Gb or higher\n", gb);
			return(-1);
		...
		*h5vars=(struct hdf5_vars*)malloc(filecount*sizeof(struct hdf5_vars));
		...
		//allocate value and row name read buffers
		datasize=sizeof(float**)+pre->dimensions[1]*sizeof(float*)+pre->dimensions[1]*pre->row_dim*sizeof(float);
		readbuff->val_buffer=(float***)malloc(datasize);
	...
	for(i=0;i<filecount;i++) {
		...
		sprintf(name, "%s/%s.%03d.h5", par->h5file_base, par->h5file_base, (int)i);
		(*h5vars)[i].file=H5Fcreate(name, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
		free(name);
		//create row dataspace
		(*h5vars)[i].row_dataspace=H5Screate_simple(1, &((*dvars)[i].vals_dataspace_dims[pre->growdim]), NULL);
		(*h5vars)[i].row_memspace=H5Screate_simple(1, &((*dvars)[i].vals_memspace_dims[pre->growdim]), NULL);
		...
		H5Pset_chunk((*h5vars)[i].row_prop_list, 1, &((*dvars)[i].vals_chunk_dims[0]));
		H5Pset_deflate((*h5vars)[i].row_prop_list, 4);
		(*h5vars)[i].row_dataset=H5Dcreate((*h5vars)[i].file, pre->rowlabel, (*h5vars)[i].row_datatype, (*h5vars)[i].row_dataspace, H5P_DEFAULT, (*h5vars)[i].row_prop_list, H5P_DEFAULT);
		//create column dataset and write to all files
		(*h5vars)[i].col_dataspace=H5Screate_simple(1, &((*dvars)[i].vals_dataspace_dims[pre->fixdim]), NULL);
		...
		H5Pset_chunk((*h5vars)[i].col_prop_list, 1, &((*dvars)[i].vals_chunk_dims[0]));
		H5Pset_deflate((*h5vars)[i].col_prop_list, 4);
		(*h5vars)[i].col_dataset=H5Dcreate((*h5vars)[i].file, pre->collabel, (*h5vars)[i].col_datatype, (*h5vars)[i].col_dataspace, H5P_DEFAULT, (*h5vars)[i].col_prop_list, H5P_DEFAULT);
		status=H5Dwrite((*h5vars)[i].col_dataset, (*h5vars)[i].col_datatype, H5S_ALL, H5S_ALL, H5P_DEFAULT, &(pre->colnames[total*pre->fixdim]));
		...
		(*h5vars)[i].vals_dataspace=H5Screate_simple(2, (*dvars)[i].vals_dataspace_dims, NULL);
		(*h5vars)[i].vals_memspace=H5Screate_simple(2, (*dvars)[i].vals_memspace_dims, NULL);
		(*h5vars)[i].vals_datatype=H5Tcopy(H5T_NATIVE_FLOAT);
		H5Tset_order((*h5vars)[i].vals_datatype, H5T_ORDER_LE);
		(*h5vars)[i].vals_dataset=H5Dcreate((*h5vars)[i].file, "values", (*h5vars)[i].vals_datatype, (*h5vars)[i].vals_dataspace, H5P_DEFAULT, (*h5vars)[i].vals_prop_list, H5P_DEFAULT);		
		(*h5vars)[i].written=0;
	}
	Rcpp::Rcout << "done.\n";
	return(filecount);
...
static int read_write_rownames_values(FILE *datafile, gzFile gzdatafile, struct read_buffers *readbuff, struct hdf5_vars *h5vars, struct dim_vars *dv, struct fastR_user_params *par, int filecount, struct preprocess_info *pre) {
	size_t f, fmin, x, y, i, j, k, l, iadj, idim, jdim, vcfbuffmax, *indptr, *predptr, *fptr;
	float **dstarts, val;
	...
	while(nread!=-1) {
		...
		i++;
		x++;
		//ensure each line contains the expect field count; fill remainder of buffer with NAN
		if(j!=0) {
			...
			i++;
				...
				fprintf(stderr, "Warning: more fields than expected found in input file line %zu\n", iadj+i+skip);		
		}
		...
		if(i==pre->row_dim || x==dv[fmin].vals_dataspace_dims[idim]) {
			for(k=fmin;k<f;k++) {
				...
				h5vars[k].written=1;
				H5Sselect_hyperslab(h5vars[k].vals_memspace, H5S_SELECT_SET, position_zero, NULL, dv[k].vals_memspace_dims, NULL);
				H5Sselect_hyperslab(h5vars[k].vals_dataspace, H5S_SELECT_SET, dv[k].vals_hyperslab_pos, NULL, dv[k].vals_memspace_dims, NULL);
				status=H5Dwrite(h5vars[k].vals_dataset, H5T_NATIVE_FLOAT, h5vars[k].vals_memspace, h5vars[k].vals_dataspace, H5P_DEFAULT, dstarts[k]);
				if(status<0) {
				...
				H5Sselect_hyperslab(h5vars[k].row_memspace, H5S_SELECT_SET, &(position_zero[0]), NULL, &(dv[k].vals_memspace_dims[pre->growdim]), NULL);
				H5Sselect_hyperslab(h5vars[k].row_dataspace, H5S_SELECT_SET, &(dv[k].vals_hyperslab_pos[pre->growdim]), NULL, &(dv[k].vals_memspace_dims[pre->growdim]), NULL);
				status=H5Dwrite(h5vars[k].row_dataset, h5vars[k].row_datatype, h5vars[k].row_memspace, h5vars[k].row_dataspace, H5P_DEFAULT, readbuff->row_buffer);
				if(status<0) {
	...
	//at end of file, check for any unwritten data, write and modify data set dimensions as necessary
	if(done==0) {
		l=0;
		...
		if(l!=(x+skip)) {
			fprintf(stderr, "Warning: fewer data entries found in input file than expected,\n   contents may have changed during processing\n");
		}
		if(i>0) {
			for(k=fmin;k<f;k++) {
				h5vars[k].written=1;
				...
				H5Dset_extent(h5vars[k].row_dataset, &dv[k].vals_dataspace_dims[pre->growdim]);
				H5Sselect_hyperslab(h5vars[k].vals_memspace, H5S_SELECT_SET, position_zero, NULL, dv[k].vals_memspace_dims, NULL);
				H5Sselect_hyperslab(h5vars[k].vals_dataspace, H5S_SELECT_SET, dv[k].vals_hyperslab_pos, NULL, dv[k].vals_memspace_dims, NULL);
				status=H5Dwrite(h5vars[k].vals_dataset, H5T_NATIVE_FLOAT, h5vars[k].vals_memspace, h5vars[k].vals_dataspace, H5P_DEFAULT, dstarts[k]);
				if(status<0) {
...
static void final_cleanup(struct read_buffers *readbuff, struct hdf5_vars **h5vars, struct dim_vars **dv, struct preprocess_info *pi, struct fastR_user_params *par, int filecount) {
	size_t i;
	char *name;
...
static int execute_fastR_hdf5convert(struct fastR_user_params *up) {
	...
	flag=read_write_rownames_values(infile, gzinfile, &rb, h5v, dims, up, nfiles, &pre);
	final_cleanup(&rb, &h5v, &dims, &pre, up, nfiles);
	if(up->gz==1) {
		gzclose(gzinfile);
	} else {
...
extern "C" SEXP fastR_hdf5convert_cpp(SEXP dataFile, SEXP h5File, SEXP headerRow, SEXP idCol, SEXP dataCol, SEXP buffSize, SEXP transpose, SEXP chunkEdge, SEXP vcf, SEXP delim, SEXP gz, SEXP poiPerFile, SEXP singleFile, SEXP serverThreads, SEXP serverMem) {
	struct fastR_user_params par;
	int retval;
	SEXP result=Rf_protect(Rf_allocVector(LGLSXP, 1));
	par.infile_name=strdup(R_CHAR(Rf_asChar(dataFile)));
	par.h5file_base=strdup(R_CHAR(Rf_asChar(h5File)));
	par.poi_per_file=Rf_asInteger(poiPerFile);
	par.header_row=Rf_asInteger(headerRow);
	par.name_column=Rf_asInteger(idCol);
	par.data_column=Rf_asInteger(dataCol);
	par.data_buffer_max=(size_t)(Rf_asReal(buffSize)*1024*1024*1024);
	par.transpose=Rf_asInteger(transpose);
	par.chunk_edge=Rf_asInteger(chunkEdge);
	par.vcf=Rf_asInteger(vcf);
	par.delim=strdup(R_CHAR(Rf_asChar(delim)));
	par.gz=Rf_asInteger(gz);
	par.single=Rf_asInteger(singleFile);
	par.server_threads=Rf_asInteger(serverThreads);
	par.server_memory=Rf_asReal(serverMem);
