...
static int read_write_rownames_values(
  ...
  if (done == 0) {
    ...
    if (l != (x + skip)) {
      fprintf(stderr,
              "Warning: fewer data entries found in input file than "
              "expected,\n   contents may have changed during processing\n");
    }
    if (i > 0) {
      for (k = fmin; k < f; k++) {
...
char* characterVectorToCString(Rcpp::CharacterVector vec) {
  // Calculate the total length of the concatenated string
  size_t totalLength = 0;
  for (int i = 0; i < vec.size(); ++i) {
    totalLength += vec[i].size();
  }
  
  // Allocate memory for the concatenated string
  char* result = new char[totalLength + 1];
  
  // Concatenate the elements of the vector into the result string
  size_t currentPosition = 0;
  for (int i = 0; i < vec.size(); ++i) {
    std::string currentString = Rcpp::as<std::string>(vec[i]);
    std::copy(currentString.begin(), currentString.end(), result + currentPosition);
    currentPosition += currentString.size();
  }
...
// [[Rcpp::export]]
int fastR_hdf5convert_cpp(SEXP dataFileSEXP, SEXP h5FileSEXP, SEXP headerRowSEXP,
                           SEXP idColSEXP, SEXP dataColSEXP, SEXP buffSizeSEXP,
                           SEXP transposeSEXP, SEXP chunkEdgeSEXP, SEXP vcfSEXP,
                           SEXP delimSEXP, SEXP gzSEXP, SEXP poiPerFileSEXP,
                           SEXP singleFileSEXP, SEXP serverThreadsSEXP, SEXP serverMemSEXP) {
  struct fastR_user_params par;
  int retval;
  bool result = false;
  Rcpp::CharacterVector dataFile(dataFileSEXP);
  Rcpp::CharacterVector h5File(h5FileSEXP);
  Rcpp::CharacterVector delim(delimSEXP);
  par.infile_name = characterVectorToCString(dataFile);
  par.h5file_base = characterVectorToCString(h5File);
  par.poi_per_file = Rcpp::as<int>(poiPerFileSEXP);
  par.header_row = Rcpp::as<size_t>(headerRowSEXP);
  par.name_column = Rcpp::as<size_t>(idColSEXP);
  par.data_column = Rcpp::as<size_t>(dataColSEXP);
  par.data_buffer_max = Rcpp::as<size_t>(buffSizeSEXP) * 1024 * 1024 * 1024;
  par.transpose = Rcpp::as<int>(transposeSEXP);
  par.chunk_edge = Rcpp::as<hsize_t>(chunkEdgeSEXP);
  par.vcf = Rcpp::as<int>(vcfSEXP);
  par.delim = characterVectorToCString(delim);
  par.gz =  Rcpp::as<int>(gzSEXP);
  par.single = Rcpp::as<int>(singleFileSEXP);
  par.server_threads = Rcpp::as<int>(serverThreadsSEXP);
  par.server_memory = Rcpp::as<float>(serverMemSEXP);
