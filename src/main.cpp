...
//     ) {
//     FRMatrix poi_matrix;
//     H5File poi_child(config.POI_file_dir);
...
void process_poi_file(
    ...
    for (int stratum = 0; stratum < stratums.nstrata; ++stratum) {
        ...
        for (int block = 0; block < num_parallel_poi_blocks; block++) {
            Rcpp::Rcout << "Processing POI block: " << block + 1 << "/" << num_parallel_poi_blocks << " for process " << process_id  + 1 << std::endl;
            int start_chunk = process_id*chunk_size;
            int end_chunk = start_chunk + chunk_size;
            if (end_chunk > num_poi) {
                end_chunk = num_poi;
...
void FastRegCpp(
    ...
    Strata stratums;
    stratums.stratify(config.split_by, covar_df, intersected_ind);
    Chunker chunker = Chunker(num_poi, intersected_ind.size(), config.max_threads, config.poi_block_size, num_poi_files);
    ...
    std::vector<bool> has_completed(num_processes, false);
    
    int num_processes_started = 0;
    int num_processes_completed = 0;

    while(num_processes_completed < num_poi_files) {
        while(num_processes_started < max_processes && num_processes_started - num_processes_completed < num_processes) {
