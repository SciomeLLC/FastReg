...
arma::mat manual_svd(arma::mat x, double& rankk, double epsilonn = 0.0001) {
  ...
  //ones.elem(arma::find(s < (epsilonn * max(s)))).fill(0.0); //remove bad eigenvals
...
void output_1d(const uword& value, uword offset, double& df, double& estimat,
               double& MSE, double inv_var, mat& output, const double& logg10, const double& logg2){
  output(value, offset) = df;
  output(value, offset + 1) = estimat;
  double se = sqrt(MSE / inv_var);
  output(value, offset + 2) = se;
  // output(value, offset + 3) = std::max(0.0,((R::pt(abs(estimat/se), df, 0, 1) + logg2)/logg10));
  output(value, offset + 3) = (R::pt(abs(estimat/se), df, 0, 1) + logg2)/logg10;
  return;
}

/// @brief Outputs 2-D estimates of betas (for G, G^2 by construction) in regressions
/// @param value Row number
/// @param offset Starting place for outputting this regression
/// @param df Degrees of freedom
...
void output_2d(const uword& value, uword offset, double& df, vec& estimat,
               double& MSE, mat& var_mat, mat& output, const double& logg10, const double& logg2){
  output(value, offset) = df;
  output(value, offset + 1) = estimat[0];
  double se = sqrt(MSE * var_mat(0,0));
  output(value, offset + 2) = se;
  output(value, offset + 3) = std::max(0.0,(R::pt(abs(estimat[0]/se), df, 0, 1) + logg2)/logg10);
  se = sqrt(MSE * var_mat(1,1));
  output(value, offset + 4) = estimat[1];
  output(value, offset + 5) = se;
  output(value, offset + 6) = std::max(0.0,(R::pt(abs(estimat[1]/se), df, 0, 1) + logg2)/logg10);
  return;
}
...
void regress_out_X(vec& yadj, vec& gadj, vec& hadj, mat& temp_matmul, 
                   mat& x, mat& Xadj, vec& w){
...
void printer(std::string& file_name, mat& output, const bool& append, const std::vector<std::string>& vnames){
...
void get_ygxw(uword& i,const uword& value, vec& y, vec& g, mat& x, vec& w, const mat& Y, const mat& G, const mat& X, vec& mask){
  y = Y.col(i);
  x = X;
  g = G.col(value);
...
/// @param pca_var_explained -- Double describing how much explained variance in X needed (for rank reduction)
/// @return 1 if successful
int FastVLA_cpp_internal(const arma::mat& Y, const arma::mat& G,
                          const arma::mat& X_orig, const std::string& dir, const arma::mat& Ybad,
                          const std::vector<std::string> cnames, const std::vector<std::string> vnames, const std::string suffix, 
                          const double epss = 1e-6, const double& mafthresh = 0.01, const bool& append = false,
                          const double pca_var_explained = 0.95){
  ...
  for(uword i = 0; i < Y.n_cols; i++){
    ...
    X = standardize_and_derank(X, pca_var_explained, double(X.n_rows - nansxy.n_elem));
    
    //loop over POIs/genes and check if good gene, bad gene, or minimal gene (only 2 unique values)
    for(uword j = 0; j < G.n_cols; j++){
      ...
      condition = -1.0 * ((caf < mafthresh) | (caf > (1.0 - mafthresh))); //1 is all NANs
      ps[j] = p;
    ...
    //do 1D G
#pragma omp parallel for private(x,w,g_unscaled, Xadj,temp_matmul,yadj,gadj,hadj,Xg,Vg,SST,SSE,MSE,df,z,F,est,ans,estt)
    for(const auto &value:oneg){
      ...
      try{
        ...
        // se = sqrt(MSE * Vg(1,1));
        output_1d(value, 9, df, est[1], MSE, 1.0 / Vg(1,1), output, logg10, logg2);
        ...
        //add in F-stat (importance of G in VLA variance regression) and SST,SSE
        F = ((SST - SSE) / 1.0) / MSE;
        output(value,29)=SST;
        output(value,30)=SSE;
        output(value,31)=F;
...
//' This function is the exported wrapper that can be called from R. It has the inputs,
//' and chunks G (the POIs) based on chunk_size to be able to stay within RAM requirements.
//' @useDynLib FastVLA 
//' @importFrom Rcpp sourceCpp
//' @export
// [[Rcpp::export]]
int FastVLA_chunked_sota(const arma::mat& Y, SEXP Gptr,const arma::ivec& v_index,const arma::ivec& i_index,
                     const arma::mat& X, const int& chunk_size, const std::string& dir, 
                     const std::vector<std::string> cnames, const std::vector<std::string> vnames, const std::string suffix, 
                     const double epss = 1e-6, const double& mafthresh = 0.005, const double pca_var_explained = 0.95){
   ...
   // std::vector<std::string>::const_iterator last = vnames.begin() + chunk_size;
   std::vector<std::string> vnames_subset(vnames.begin(), vnames.begin() + chunk_size);
   //do first chunk
   arma::mat G = scanBEDMatrix(Gptr, i_index, v_index.rows(0, chunk_size-1));
   int a = FastVLA_cpp_internal(Y, G, X, dir, xy_bools, cnames, vnames_subset, suffix, epss, mafthresh, false, pca_var_explained);
   //do rest of chunks, appending output
   //#pragma omp parallel for
   for(int i = 1; i < num_chunks; i++){
     G = scanBEDMatrix(Gptr, i_index, v_index.rows(chunk_size*i, chunk_size*(i+1) - 1));
     // first = vnames.begin() + i*chunk_size;
     // last = vnames.begin() + (i+1)*chunk_size - 1;
     vnames_subset.assign(vnames.begin() + i*chunk_size, vnames.begin() + (i+1)*chunk_size);
     a = std::min(a, FastVLA_cpp_internal(Y, G, X, dir, xy_bools, cnames, vnames_subset, suffix, epss, mafthresh, true, pca_var_explained));
   }
   //do last chunk if needed, appending output
   if((int)v_index.n_elem > chunk_size * num_chunks){
     arma::mat G = scanBEDMatrix(Gptr, i_index, v_index.rows(chunk_size*num_chunks, v_index.n_elem - 1));
     // first = vnames.begin() + num_chunks*chunk_size;
     // last = vnames.end();
     std::vector<std::string> vnames_subset2(vnames.begin() + num_chunks*chunk_size, vnames.end());
     a = std::min(a, FastVLA_cpp_internal(Y, G, X, dir, xy_bools, cnames, vnames_subset2, suffix, epss, mafthresh, true, pca_var_explained));
   }
   return a;
}                           
