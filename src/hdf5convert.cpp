...
static int read_write_rownames_values(
  ...
  size_t f, x, y, i, j, k, l, fmin, iadj, idim, jdim, vcfbuffmax, *indptr,
      *predptr, *fptr;
  // unsigned int fmin, f;
  float **dstarts, val;
  ...
  while (nread != -1) {
    ...
    if (i == pre->row_dim || x == dv[fmin].vals_dataspace_dims[idim]) {
      ...
      if (x == dv[fmin].vals_dataspace_dims[idim]) {
        x = 0;
        fmin++;
        if ((int)par->transpose == 0 ||
            ((int)par->transpose == 1 && (int)fmin == filecount)) {
          done = 1;
        }
      }
    }
...
static void final_cleanup(struct read_buffers *readbuff,
  ...
  for (i = 0; (int)i < filecount; i++) {
    ...
    // if an HDF5 file was created but no data was written due to input
    // filtering, delete the file
...
int FastRegImportCpp(std::string dataFile, std::string h5File, int headerRow,
                     ...
                     int poiPerFile, bool singleFile, int serverThreads,
                     float serverMem) {
  struct fastR_user_params par;
  int retval;
