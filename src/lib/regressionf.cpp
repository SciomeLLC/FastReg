...
arma::fcolvec t_dist_r(arma::fcolvec abs_z, int df) {
  ...
  for (size_t i = 0; i < abs_z.size(); i++) {
    ret_val(i) = static_cast<float>(
        -1 * (R::pt(abs_z(i), df, true, true) + log(2)) / log(10));
  }
...
float chisq(float lrs, int df) {
  return static_cast<float>(R::pchisq(lrs, df, 0, 1) / (-1.0 * log(10)));
}
...
void LogisticRegression::run_vla_2(arma::fmat &cov, arma::fmat &pheno,
  ...
  for (int poi_col_idx : poi_2_idx) {
    ...
    beta = Eigen::VectorXf::Zero(result.num_parms, 1);
    beta_old = beta;
    cov_w_mat =
        Eigen::Map<Eigen::MatrixXf>(cov.memptr(), cov.n_rows, cov.n_cols);
    ...
    POI = Eigen::Map<Eigen::MatrixXf>(poi_col.memptr(), poi_col.n_rows, 1);
    Eigen::MatrixXf temp_mat_e = int_w_mat;
    int_w_mat = temp_mat_e.array().colwise() * POI.col(0).array();
    X = Eigen::MatrixXf::Zero(int_w_mat.rows(), cov_w_mat.cols() + 1);
    ...
    beta2 = Eigen::VectorXf::Zero(n_parms2, 1);
    beta_old2 = beta2;
    ...
    X2 = Eigen::MatrixXf::Zero(int_w_mat2.rows(),
    ...
    ll1_rel_err = 1.0;
    for (int iter = 0; iter < max_iter && ll1_rel_err > epss; iter++) {
      // Fit 1
    ...
    // Fit 2
    ll2_rel_err = 1.0;
    for (int iter = 0; iter < max_iter && ll2_rel_err > epss; iter++) {
    ...
    temp_se2 = arma::fcolvec(diag2.data(), diag2.size(), true, false);
...
//     pval2)
  for (int poi_col_idx : poi_3_idx) {
    checkInterrupt();
    A = Eigen::MatrixXf::Zero(result.num_parms, result.num_parms);
    A(0, 0) = 1;
    A(result.num_parms, result.num_parms) = 10;
    beta = Eigen::VectorXf::Zero(result.num_parms, 1);
    beta_old = beta;
    ...
    POI_sqrd = Eigen::Map<Eigen::MatrixXf>(poi_col_sqrd.memptr(),
                                           poi_col_sqrd.n_rows, 1);
    A2 = Eigen::MatrixXf::Zero(n_parms2, n_parms2);
    A2(0, 0) = 1;
    A2(n_parms2, n_parms2) = 10;
    beta2 = Eigen::VectorXf::Zero(n_parms2, 1);
    beta_old2 = beta2;
