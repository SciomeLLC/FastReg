...
FRMatrix CovariateMatrix::create_design_matrix()
{
  ...
  {
    cov.create_matrix(reader.tokenized, reader.row_names);
...
int CovariateMatrix::find_col_idx(std::string col_name)
{
  ...
  Rcpp::Rcout << "WARNING: column name: " << col_name << " not found in file " << file_name << std::endl;
  return -1;
}
...
void CovariateMatrix::set_cov_col_idx()
{
  for (size_t i = 0; i < covariates.size(); i++)
  {
    ...
    {
      Rcpp::stop("ERROR: Covariate column name '%s' not found in %s", cov_name, file_name);
    }
  }
}
