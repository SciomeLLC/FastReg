...
int Chunker::get_num_workers() { return num_workers; }

int Chunker::get_openmp_threads() { return num_threads; }

int Chunker::get_chunk_size() { return chunk_size; }

int Chunker::get_total_workers() { return _num_files; }
...
double Chunker::get_data_size() {
  ...
  double data_size = std::exp(std::log(matrix_size) + std::log(float_size) +
                     ...
                     master_thread_memory;
  return data_size;
}
void Chunker::estimate_chunks() {
  this->num_workers =
      std::min(std::min(_num_files, num_available_workers), _max_workers);
  ...
  if (os == "Darwin") {
    ...
    if (_max_threads*num_available_workers > num_available_workers*2) {
      cpp11::warning("# openmp threads * cores exceeds the amount supported for hyperthreading. This will degrade performance!");
...
ChunkConfig Chunker::estimate_num_files(int num_poi, int num_ind,
  ...
  // total available threads minus 1 to run OS and 1 to run fastR main process
  // divided by count of BLAS threads per process
  ...
  if (usr_mem != -1.0) {
    max_mem = static_cast<unsigned long long>((usr_mem * 1024 * 1024 * 1024) -
                                              524288000ULL);
  } else {
  ...
  chunk_config.num_files =
      (num_poi / max_chunk) + ((num_poi % max_chunk) > 0 ? 1 : 0);
