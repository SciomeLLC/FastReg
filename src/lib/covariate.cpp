...
void Covariate::set_col_idx(int idx)
{
  col_idx = idx;
}
...
FRMatrix Covariate::create_numeric_matrix(std::vector<std::string> col_vals)
{
  ...
  for (size_t row = 0; row < col_vals.size(); row++)
  {
    ...
    catch (const std::invalid_argument &e)
    {
      ...
      {
        Rcpp::stop("Expected numeric value in column: %s but found invalid string '%s'", name, col_vals[row]);
      }
    }
  }
...
FRMatrix Covariate::create_categorical_matrix(std::vector<std::string> col_vals)
{
  FRMatrix candidate_mat;
...
void Covariate::create_matrix(std::vector<std::vector<std::string>> tokenized, std::vector<std::string> row_names)
{
  ...
  // Generate unique covariate levels if categorical and user-specified levels are empty
