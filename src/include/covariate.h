...
/**
 * @brief The Covariate class manages individual covariates in a regression
 * model.
 *
 * It handles both numeric and categorical covariates, including creating
 * matrices, generating levels for categorical variables, and standardizing
 * numeric variables.
 */
class Covariate {
public:
   ...
   * @param cov_standardize Boolean flag to indicate if standardization is
   * required.
   */
  Covariate(std::string cov, std::string covar_type, std::string cov_ref_level,
            std::string cov_levels, bool cov_standardize) {
    name = cov;
    cov_type = covar_type;
   ...
   */
  void create_matrix(std::vector<std::vector<std::string>> values,
                     std::vector<std::string> row_names);
  /**
   * @brief Filters collinear covariate columns based on the specified R-squared
   * threshold.
   *
...
private:
  ...
  /**
   * @brief Generates unique levels for a categorical covariate if none are
   * provided.
   *
