...
/**
 * @brief The Reader class is an abstract base class for reading data from
 * various file types.
 *
 * This class defines a common interface for accessing datasets such as
 * individuals and names (predictors of interest), and reading chunks of data.
 * It is designed to be extended by specific implementations, like HDF5-based
 * readers.
 */
class Reader {
public:
   ...
   *
   * This function is pure virtual and must be implemented by any class
   * inheriting from Reader.
   *
  ...
  /**
   * @brief Reads a chunk of data from the dataset for specified rows and
   * columns.
   *
   * This function is pure virtual and must be implemented by any class
   * inheriting from Reader.
...
/**
 * @brief The POI class provides an interface to read predictors of interest
 * (POI) from HDF5 files.
 *
 * This class extends the Reader class and provides the functionality to read
 * individuals, names, and chunks of data from HDF5 files. It handles various
 * HDF5 types and ensures memory is managed correctly.
 */
...
public:
   ...
   *
   * @param read_only Flag to indicate if the file should be opened in read-only
   * mode.
   */
   ...
   *
   * Defines the size of the memory space to be used when reading chunks of data
   * from the file.
   *
  ...
  /**
   * @brief Reads a chunk of data from the HDF5 file for specified rows and
   * columns.
   *
   * @param rows A vector of row identifiers to read.
   * @param cols A vector of column identifiers to read.
