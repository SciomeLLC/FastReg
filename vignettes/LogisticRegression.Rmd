...
# Logistic Formula: bin.resp ~ 1 + Age + Sex + Treatment + Geno
# Geno = column added to the covariates
# Assuming an additive model 

# create one vignette that shows interactions vs not (covariate v genotype)
summary_file <- "POI_Summary_1_stratum_1.tsv"
summary_df <- read.table(summary_file, n=6, header=TRUE, sep="\t", check.names = FALSE, stringsAsFactors = FALSE)
print(summary_df)

results_file <- "Results_stratum_1.tsv"
