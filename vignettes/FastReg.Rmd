...
If you want to control the number of files created - you can set the parameter `poi.per.file` where the function will divide the POI text data into `n` files with `poi.per.file` POIs each.

```{r include=TRUE, eval=FALSE}
library(FastReg)
TextToH5("testdata", "./FastRegData", delimiter="\t", poi.per.file=10000)
```
